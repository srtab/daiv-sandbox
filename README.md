...
| **DAIV_SANDBOX_RUNNING_SESSION_MAX_IDLE_SECONDS** | Age (since the last request touched it) after which a _running_ session container is removed. Requires `DAIV_SANDBOX_REDIS_URL`; 0 disables.                                                                                                                                                                                                               | Default: 14400 (4h)                                                         |
...
A successful text `read` also reports its line window, so a paging client never has to re-derive one by counting the returned text — which would also count the truncation marker's lines. `total_lines` is the file's total source lines, `end_line` is the 1-indexed **last complete** line in `content`, and `truncated` is `true` when the byte cap cut the body. When the cap lands mid-line the text past `end_line` is a partial line, so resuming at `end_line` re-reads it whole; a single line longer than the cap yields no complete line at all, and `end_line` equals the request's `offset`. Binary reads, the empty-file sentinel, and every error leave `total_lines`/`end_line` at `null` and `truncated` at `false`.
