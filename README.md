...
| **DAIV_SANDBOX_EGRESS_CA_CERT_FILE**          | Path to the shared egress CA certificate. Installed into every egress sandbox automatically. Required when egress proxy is enabled.                                                                                                                               | Required when egress enabled                                                |
| **DAIV_SANDBOX_EGRESS_CA_KEY_FILE**           | Path to the shared egress CA private key. Provided only to sidecars, never to the sandbox. Required when egress proxy is enabled.                                                                                                                                 | Required when egress enabled                                                |
