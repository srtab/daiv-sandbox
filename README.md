...
| **DAIV_SANDBOX_NETWORK**                      | Upstream Docker network the egress sidecar's NIC joins for outbound connectivity; falls back to Docker's default bridge when unset. The sandbox is never attached to it directly — network-enabled sessions reach the internet only through the proxy. Sessions without networking stay isolated (`network_mode=none`). | Optional                                                                    |
...
| **DAIV_SANDBOX_EGRESS_CA_CERT_FILE**          | Path to the shared egress CA certificate. Installed into every egress sandbox automatically. Required when egress proxy is enabled.                                                                                                                                                                                     | Required to enable network egress (set both, or neither)                    |
| **DAIV_SANDBOX_EGRESS_CA_KEY_FILE**           | Path to the shared egress CA private key. Provided only to sidecars, never to the sandbox. Required when egress proxy is enabled.                                                                                                                                                                                       | Required to enable network egress (set both, or neither)                    |
