...
- Bumped `ipython` to 9.17.0 (no backwards-incompatible changes; not imported by the app), `pydantic` to 2.13.5, `sentry-sdk` to 2.68.1, `ruff` to 0.16.5, `pyproject-fmt` to 2.29.0 (formatting refinement; no reformatting of `pyproject.toml`), and `prek` to 0.5.0 (breaking changes checked — no `@`-prefixed group names, `PREK_MAX_CONCURRENCY`, `init-template-dir`, or `auto-update` in use).
- Bumped `docker` to 7.2.0 and `pydantic` to 2.13.4.
