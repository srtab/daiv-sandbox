...
- `POST /session/{id}/fs/{op}` — Python-free workspace file operations (`ls`, `read`, `grep`, `glob`, `write`, `edit`, `delete`) that act anywhere under `/workspace`. `read` paginates text with `offset`/`limit` and returns binary as base64; `grep` runs a regular-expression search (see below); `glob` supports `*`, `**`, `?`, and `[abc]`.
- `fs/grep` is regex-backed: it runs a static `ripgrep` binary injected into the task container at session start (one bundled per supported arch), falling back to POSIX `grep -E` (ERE) on images/arches where ripgrep is unavailable. New request fields `case_insensitive`, `multiline` (ripgrep only), and `head_limit` (cap on returned matches). `pattern` is always a regular expression — metacharacters must be escaped to match literally — and an uncompilable pattern returns `error.code=invalid_pattern` with the engine's message.
